    ...
    useEffect(() => {
        ...
        try{
            if(window.opener && !window.opener.closed){
                // Post message back to the opener window (the main application)
                window.opener.postMessage({type: "GOOGLE_AUTH_COMPLETE", connected, state}, window.location.origin);
...
// NOTE: You might need to define 'animate-bounce-once' and 'animate-pulse-short' in your tailwind.config.js
...
module.exports = {
  theme: {
    extend: {
      animation: {
        'bounce-once': 'bounce 1s ease-out 1', // Only runs once
        'pulse-short': 'pulse 1s cubic-bezier(0.4, 0, 0.6, 1) infinite', // For the loading bar effect
      },
    },
  },
}
