...
import { TriggersPage } from "@/components/dashboard/triggers";
import { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import type { User } from "@/components/types";

export default function TriggerPageClient() {
    const [user, setUser] = useState<User | null>(null);
      ...
      const clearSession = async () => {
        // If you use server-set HttpOnly cookie, call logout API to clear it
        try {
      ...
      // user is null -> useEffect above will redirect; return null to avoid render blink
      if (!user) {
        return null;
      }
    
      return <TriggersPage user={user} onLogout={handleLogout} />; 
}